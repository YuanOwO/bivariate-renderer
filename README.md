...
/* 3. 渲染操作：用關鍵字 plot 包起來，指定輸出檔案名稱 */
plot( g(x, y) - 8, "output.jpg" );
```
...
- `pi`：圓周率，約等於 3.14159
- `e`：自然對數的底，約等於 2.71828
- `phi`：黃金比例，約等於 1.61803

### 函數

- **三角函數**：`sin(x)`, `cos(x)`, `tan(x)`
- **反三角函數**：`asin(x)`, `acos(x)`, `atan(x)`
- **指數與對數**：`exp(x)`, `ln(x)` (自然對數), `log(x)` (以 10 為底)
- **其他**：`sqrt(x)`, `cbrt(x)`, `abs(x)`, `max(x, y)`, `min(x, y)`
