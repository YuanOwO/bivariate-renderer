...
struct Function {
    ...
    Function() = default;
    Function(const Vector<String>& params, ExprPtr func_body) : parameters(params), body(func_body) {}
    Function(const Vector<String>& params, BuiltinFunc func) : parameters(params), builtin_func(func) {}
};
...
class Environment {
   ...
   public:
    ...
    Environment(const Vector<String>& unknowns, Environment* outerScope = nullptr)
        : outer(outerScope), unknown_variables(unknowns.begin(), unknowns.end()) {}

    void setVariable(const String name, Value value);
    void setFunction(const String name, const Vector<String>& params, ExprPtr body);
    void setBuiltinFunction(const String name, const Vector<String>& params, BuiltinFunc func);
